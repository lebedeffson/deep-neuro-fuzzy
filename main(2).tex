...
\title{Routed Kolmogorov--Arnold Fuzzy Networks for Stable Interpretable Prediction\thanks{The research was carried out within the state assignment of Ministry of Science and Higher Education of the Russian Federation (theme No. 124112200072-2).}}
...
On the \textit{covtype\_binary\_20000} benchmark, the best routed KA configuration reaches F1 $=0.8174\pm0.0101$ with 936 active routed predicates and active-rule Jaccard $=0.9167$. It is close to the strongest stacked fuzzy baseline (F1 $=0.8222\pm0.0074$) and reaches the histogram-gradient-boosting level with slightly higher mean F1 in the same focused block, while providing a much more reproducible rule structure than the stacked fuzzy model. The results support a quality--complexity--stability view of deep neuro-fuzzy design: the proposed KAFN is not a universal accuracy winner, but it gives a strong operating point where predictive quality and stable interpretability are jointly optimized.
...
The architectures are evaluated under a shared data protocol, with family-specific settings reported in the reproducibility artifacts. This makes the comparison controlled while preserving the settings required by each architecture. The evaluation protocol combines predictive quality, rule-base complexity, and reproducibility of internal rule activation, and it is complemented by a case-level internal trace. The central result is a controlled quality--complexity--stability comparison that identifies Routed KAFN as a strong stability-oriented fuzzy operating point in the focused Covtype setting.
...
\quad k=1,\dots,K,
\]
where $\mathcal{S}_k$ is a fixed concept route and $\nu_{qt}$ are fuzzy terms over first-stage concepts. In the pure two-layer form, the routed concept vector is then passed to a sparse affine decision map,
\[
\hat{y}_{\mathrm{KA}} =
...
\]
followed by low/medium/high fuzzy terms over $\pi_m(x)$. These channels are reported as named projection predicates. In the final configuration, the projection routes are fixed before training, so the structural topology remains reproducible across seeds.
...
Although the KAFN rule count is larger than in compact ANFIS-style baselines, these rules are routed one-dimensional or fixed projection predicates rather than high-order multidimensional antecedent products. Therefore, their structural cost should be interpreted together with topology stability and feature-level readability.
...
A structured reproducibility package is provided with the manuscript materials and includes resolved run configurations, seeds, metric tables, and figure-generation inputs (\href{https://github.com/lebedeffson/deep-neuro-fuzzy}{repository}, \href{https://github.com/lebedeffson/deep-neuro-fuzzy/tree/main/docs/artifacts}{artifacts}).
...
\begin{figure}[H]
    ...
    \caption{Case-level explainability flow in Routed KAFN}
    \label{fig:case_explainability_flow}
\end{figure}

In the present study, the local analysis is demonstrated on the focused \textit{covtype\_binary\_20000} setting, where Routed KAFN is the main proposed model. The first level of the architecture is dominated by routed feature predicates and optional sparse projection predicates, so the input is processed through a fixed, named rule dictionary rather than through a seed-dependent hidden rule topology. At the next level, these local signals are compressed into additive KA concept states. By the time the model reaches the output layer, the final prediction is determined by a small group of dominant affine decision contributions.

The trace fields reported for such an example are summarized in Table~\ref{tab:case_rules}.
...
Decision & \makecell[l]{\texttt{concept k or projection m} \\ \texttt{with affine coefficient}} & reported per object & final decision contribution \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\toprule
Model & \makecell[c]{Avg.\\ rules/units} & \makecell[c]{Mean\\ active-rule\\Jaccard} & Diabetes & Linnerud weight & Breast cancer & Wine binary & Digits binary\\
 &  &  & RMSE & RMSE & F1 & F1 & F1 \\
\midrule
...
\begin{tabular}{lcccc}
\toprule
Model & \makecell[c]{Avg.\\ rules/units} & \makecell[c]{Mean active-rule\\Jaccard} & California housing & Covtype binary \\
 &  &  & RMSE & F1 \\
\midrule
...
\toprule
Model & F1 & ROC-AUC & PR-AUC & \makecell[c]{Active rules/\\predicates} & Jaccard & Comment \\
\midrule
...
For the pre-projection teacher-guided grouped KA model, the additional three-seed verification gives ROC-AUC $=0.8848$ and PR-AUC $=0.8595$ at F1 $=0.8166\pm0.0029$ with 936 active predicates and active-rule Jaccard $=0.9079$. This confirms that the routed additive construction is not only an F1-specific improvement. The binary-aware variant then reduces the active predicate set from 936 to 780 and increases Jaccard to $0.9188$ with essentially unchanged F1. The fixed-projection variant is selected as the final KAFN because it gives the best mean F1 among the stable KA configurations while keeping Jaccard above $0.91$.
...
\begin{tabular}{lcccccl}
\toprule
Variant & F1 & ROC-AUC & PR-AUC & \makecell[c]{Active rules/\\predicates} & Jaccard & Interpretation \\
\midrule
...
\toprule
Model & F1 & ROC-AUC & PR-AUC & Rules/units & Active-rule Jaccard \\
\midrule
...
Routed KAFN, final config & 0.8485$\pm$0.0030 & 0.9246$\pm$0.0036 & 0.9135$\pm$0.0043 & 936 & 0.9696 \\
\bottomrule
\end{tabular}%
}
\end{table}

The capacity-tuned stacked model substantially improves over the smaller reference configurations and becomes stronger than histogram gradient boosting, MLP, and the tested hierarchical fuzzy variant on full Covtype. However, it remains below Random Forest and Extra Trees. DFFL in the current three-seed run reaches F1 $=0.8478\pm0.0037$, which is above histogram gradient boosting but below MLP, stacked, and tree ensembles. Routed KAFN reaches a similar full-Covtype F1 level, $0.8485\pm0.0030$, while preserving much higher active-rule stability ($0.9696$).

The full-Covtype KAFN run uses the same final routed configuration as the focused Covtype experiment and is reported as a large-scale validation check rather than as a new capacity-tuned variant.
...
As a secondary large-scale sanity check, we evaluate the 10 percent KDDCup99 binary split. In this check, fuzzy models reach near-saturated F1 values: shallow $0.9965\pm0.0006$, stacked $0.9995\pm0.0001$, hierarchical $0.9995\pm0.0001$, DFFL $0.9993\pm0.0001$, and Routed KAFN $0.9996\pm0.0000$. This setting confirms feasibility and repeated-run behavior, but it is weakly discriminative because most methods approach perfect scores. For this reason, we do not use KDD as a primary quality separator in the main comparative interpretation.
...
Routed KAFN, final config & 0.7758$\pm$0.0018 & 0.8729$\pm$0.0010 & 0.8773$\pm$0.0006 & 3 \\
...
On SUSY-200k, Routed KAFN gives the strongest three-seed fuzzy F1 in this block, while histogram gradient boosting and MLP retain slightly higher single-seed ROC-AUC or PR-AUC values. We therefore use SUSY as a discriminative large-scale check, but not as a claim of universal dominance over external baselines.
...
These tests are computed on the reference three-seed benchmark blocks and do not include the separate KAFN-focused Covtype ablations or the later large-scale KAFN validation checks. The separate full-Covtype capacity table is also treated as a diagnostic and excluded from significance testing.

Because active-rule stability depends on the activation threshold $\tau_{\mathrm{act}}$, we additionally ran a threshold-sensitivity check for the reference fuzzy architectures on the main benchmark block with $\tau\in\{0.3,0.5,0.7\}$. At $\tau=0.3$, mean active-rule Jaccard across datasets is 0.4511 (stacked), 0.4381 (hierarchical), and 0.2261 (DFFL). At the default $\tau=0.5$, the same values are 0.1218, 0.1640, and 0.2198. At $\tau=0.7$, stacked and hierarchical saturate at 1.0 in this fast setting due to the very sparse surviving active-rule sets, while DFFL remains at 0.2090. This supplementary fast-setting check is threshold-sensitive, does not include Routed KAFN, and does not replace the main reference comparison in Table~\ref{tab:main_results}. We therefore keep $\tau_{\mathrm{act}}=0.5$ as the default reporting point and interpret high-$\tau$ values with caution.
...
Thus, Routed KAFN can be read as a response to the two main weaknesses of the DFFL profile observed in the reference experiments: unstable active-rule recovery and loss of cross-feature information after early local aggregation.
...
The third limitation concerns large-scale efficiency. The large-scale results are mixed. Routed KAFN remains stable on full Covtype, SUSY-200k, and KDDCup99-10\%, but full Covtype also shows that the final routed configuration does not close the gap to capacity-tuned stacked fuzzy models or tree ensembles. Scaling and tuning Routed KAFN on additional large-scale data sets, including full SUSY and full KDDCup99, remains future work.
...
The large-scale checks support the same reading. The final KAFN configuration reaches F1 $=0.8485\pm0.0030$ on full Covtype, F1 $=0.7758\pm0.0018$ on SUSY-200k, and F1 $=0.9996\pm0.0000$ on KDDCup99-10\%, while preserving high active-rule stability.
...
Further work should scale Routed KAFN to full Covtype and improve projection routing. Domain experts should then validate the named concepts and local traces.
