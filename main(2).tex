...
The Kolmogorov--Arnold superposition theorem gives another useful architectural viewpoint: a multivariate continuous function can be represented through sums and superpositions of univariate functions~\cite{kolmogorov1957superposition,arnold1957functions}. In this paper we do not use the theorem as a strict approximation guarantee for a finite trained fuzzy model. Instead, we use it as a design principle for replacing high-order fuzzy rules by routed sums of one-dimensional fuzzy predicates and upper-level concept functions.
...
\begin{table}[H]
\centering
\caption{Dominant feature-named rules in the case-level example for DFFL.}
...
\midrule
Stage 1 & \makecell[l]{\texttt{mean texture IS MEDIUM} \\ \texttt{AND mean area IS MEDIUM}} & 0.7774 & dominant local signal \\
Stage 1 & \makecell[l]{\texttt{mean concavity IS MEDIUM} \\ \texttt{AND mean concave points IS MEDIUM}} & 0.9987 & nearly complete block activation \\
Stage 1 & \makecell[l]{\texttt{worst radius IS MEDIUM} \\ \texttt{AND worst perimeter IS MEDIUM}} & 0.9940 & stable local concept \\
Stage 2 & \makecell[l]{\texttt{shape-contrast concept IS HIGH} \\ \texttt{AND compactness concept IS LOW}} & 0.7889 & main aggregate transition \\
Stage 2 & \makecell[l]{\texttt{mass-size concept IS LOW} \\ \texttt{AND boundary-irregularity concept IS HIGH}} & 0.8739 & dominant global state \\
Decision & \makecell[l]{\texttt{global malignancy evidence IS LOW} \\ \texttt{AND support concept IS LOW}} & 0.3554 & strongest negative contribution \\
Decision & \makecell[l]{\texttt{global malignancy evidence IS LOW} \\ \texttt{AND size-risk concept IS LOW}} & 0.3111 & second strongest contribution \\
Decision & \makecell[l]{\texttt{texture-risk concept IS HIGH} \\ \texttt{AND support concept IS LOW}} & 0.0959 & complementary decision signal \\
\bottomrule
\end{tabular}
\end{table}
...
The key quantitative evidence is reported in Tables~\ref{tab:main_results}, \ref{tab:extended_results}, \ref{tab:kanfis_covtype}, \ref{tab:kafn_ablation}, \ref{tab:covtype_capacity_check}, \ref{tab:susy_discriminative}, \ref{tab:best_vs_baseline}, and \ref{tab:all_vs_baseline}.
...
\begin{tabular}{lcccc}
\toprule
Model & F1 & Active rules & Active-rule Jaccard & Comment \\
\midrule
shallow fuzzy & 0.7645$\pm$0.0030 & 18.7 & 0.3493 & compact shallow reference \\
DFFL & 0.7840$\pm$0.0078 & 73.0 & 0.1077 & concept-oriented reference \\
hierarchical fuzzy & 0.8037$\pm$0.0058 & 64.7 & 0.2598 & grouped reference \\
HistGradientBoosting & 0.8154$\pm$0.0065 & -- & -- & non-fuzzy baseline \\
Routed KAFN, binary-aware & 0.8165$\pm$0.0067 & 780 & 0.9188 & most stable KA variant \\
Routed KAFN, fixed projections & 0.8174$\pm$0.0101 & 936 & 0.9167 & final KA variant \\
stacked fuzzy & 0.8222$\pm$0.0074 & 20.7 & 0.0978 & strongest fuzzy F1 reference \\
Extra Trees & 0.8546$\pm$0.0026 & -- & -- & teacher and upper baseline \\
\bottomrule
\end{tabular}
\end{table}
...
\toprule
Variant & F1 & Rules & Active rules & Jaccard & Interpretation \\
\midrule
Teacher-guided grouped KA & 0.8166$\pm$0.0029 & 936 & 936 & 0.9079 & strong stable base model \\
KA + calibrated threshold & 0.8169$\pm$0.0031 & 936 & 936 & 0.9079 & best threshold-only F1 \\
KA + binary-aware terms & 0.8165$\pm$0.0067 & 936 & 780 & 0.9188 & fewer rules, clearer one-hot logic \\
Routed KAFN + fixed projections & 0.8174$\pm$0.0101 & 936 & 936 & 0.9167 & final accuracy--stability trade-off \\
\bottomrule
\end{tabular}
\end{table}
...
linnerud\_weight       & RMSE & hierarchical model & 0.2430$\pm$0.1691 & hist gradient boosting & 0.3058$\pm$0.1508 \\
...
california\_housing    & RMSE & stacked model      & 0.1193$\pm$0.0066 & hist gradient boosting & 0.0972$\pm$0.0014 \\
covtype\_binary\_20000 & F1   & Routed KAFN        & 0.8174$\pm$0.0101 & extra trees classifier           & 0.8546$\pm$0.0026 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Reference neuro-fuzzy architectures against the best external baseline per data set (signed delta; positive is better). Routed KAFN is reported separately in Tables~\ref{tab:kanfis_covtype} and~\ref{tab:kafn_ablation}.}
...
linnerud\_weight       & RMSE & hist gradient boosting & -0.0462 & +0.0170 & +0.0628 & +0.0183 \\
...
california\_housing    & RMSE & hist gradient boosting & -0.0349 & -0.0221 & -0.0262 & -0.0313 \\
covtype\_binary\_20000 & F1   & extra trees classifier           & -0.1444 & -0.0417 & -0.0428 & -0.1027 \\
\bottomrule
\end{tabular}
\end{table}

As shown in Table~\ref{tab:all_vs_baseline}, the reference neuro-fuzzy models outperform the best external baseline on \textit{linnerud\_weight}. On the remaining tasks they are either below the baseline or at parity (\textit{digits\_binary} for DFFL). Routed KAFN improves the focused Covtype fuzzy result, but it remains below Extra Trees, which is used as the teacher and the strongest external upper baseline in this setting.
