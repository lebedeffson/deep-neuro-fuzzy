...
As a second large-scale check, we evaluate \textit{kddcup99\_binary\_10percent}. This setting remains useful for verifying scalability and repeated-run behavior, but it is not strongly discriminative because most methods approach perfect scores.

\begin{table}[H]
\centering
\caption{Large-scale feasibility check on \textit{kddcup99\_binary\_10percent}. Fuzzy values are mean $\pm$ std over three seeds; baseline values are a single-seed contextual snapshot (seed 23).}
...
Model & F1 \\
\midrule
shallow fuzzy & 0.9965$\pm$0.0006 \\
stacked fuzzy & 0.9995$\pm$0.0001 \\
hierarchical fuzzy & 0.9995$\pm$0.0001 \\
DFFL & 0.9993$\pm$0.0001 \\
...
To reduce the ``near-perfect'' saturation effect of KDD, we add a discriminative large-scale check on \textit{SUSY-200k}. Fuzzy-model values are reported over three seeds; baseline values are a single-seed snapshot (seed 23) and are therefore used as a contextual check, not as a significance-tested comparison block.
...
Runtime profiling on full Covtype (seed 23, matched fast settings) shows that stacked is the fastest of the three deep fuzzy variants in training (69.57s), hierarchical is intermediate (122.58s), and DFFL is the slowest (423.89s). Inference-plus-evaluation time remains in the same low-second range for all three models (about 2--3s in this setup). This supports the practical trade-off interpretation: DFFL currently has both a quality and training-time penalty relative to stacked under the tested configuration.
